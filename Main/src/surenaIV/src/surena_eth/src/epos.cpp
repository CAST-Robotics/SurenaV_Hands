...
bool Epos::SetLed( unsigned char  effect)
{
    QByteArray data;
    data.append(6);
    data.append((char)effect); //pos hi
    data.append((char)0); //pos low
    data.append((char)0);  //speed
    data.append( QByteArray(4, Qt::Initialization::Uninitialized));

   can.WriteMessage(0x211, 14,data);
return true;
}
...
int32_t  Epos::ReadRegisterValue(int index,int subIndex,int canID, int devID,int timeout)throw(std::runtime_error)
{
        ...
        return value;
         ...
         QLOG_TRACE()<<"errot in dev:"<<devID<<" can:"<<canID;
        throw std::runtime_error(std::string("Read Register Erorr"));
        return value;
...
//========================================================================
inline QByteArray Epos::CreateLedPacket(char commanad)
{
    QByteArray data;
    data.append(0x02);
    data.append(0x11);
    data.append(6);
    data.append((char)commanad); //pos hi
    data.append((char)0); //pos low
    data.append((char)0);  //speed
    data.append( QByteArray(4, Qt::Initialization::Uninitialized));
    return data;
...
//static int first=0;
   ...
   command.append(CreateDynamixelPacket(0x211,4,motorPositions.at(29),40));//waist dx
     ...
     command.append(CreateDynamixelPacket(0x211,2,motorPositions.at(31),40));
        ...
        command.append(CreateDynamixelPacket(0x211,6,motorPositions.at(33),40));

    }
//    else if(state==6){


//        command.append(CreateLedPacket(Led));//led
...
EPOSErrors Epos::Init(int tryCount)
{
    ...
    qDebug()<<"ping";
    while(! ping.Start("192.168.0.10") && pingCount<tryCount )
    {
        QThread::msleep(500);
