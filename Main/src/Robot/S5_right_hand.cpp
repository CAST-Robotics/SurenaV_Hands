...
right_hand::right_hand(VectorXd q_ra,VectorXd r_target,MatrixXd R_target)
{
    ...
    sai_target=euler_calc(R_target)(0);
    theta_target=euler_calc(R_target)(1);
    phi_target=euler_calc(R_target)(2);
    Eul_Angles_target <<sai_target,theta_target,phi_target;
...
theta=euler_calc(R_right_palm)(1);
sai=euler_calc(R_right_palm)(0);
phi=euler_calc(R_right_palm)(2);
/*
theta_BF=theta;
sai_BF=sai;
phi_BF=phi;
theta_dot = (theta - theta_BF)/T;
sai_dot = (sai - sai_BF)/T;
phi_dot = (phi - phi_BF)/T;
...
*/
w_right_palm<<-sin(phi)*theta_dot+cos(theta)*cos(phi)*sai_dot,
               cos(phi)*theta_dot+cos(theta)*sin(phi)*sai_dot,
               phi_dot-sin(theta)*sai_dot;
